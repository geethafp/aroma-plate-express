  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto max-w-2xl px-4 py-8">
        <button
          onClick={() => step === 'cart' ? navigate('/') : step === 'address' ? setStep('cart') : setStep('address')}
          className="mb-6 flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors"
        >
          <ArrowLeft size={16} /> {step === 'cart' ? 'Back to menu' : step === 'address' ? 'Back to cart' : 'Back to address'}
        </button>

        {/* Progress */}
        <div className="flex items-center gap-2 mb-8">
          {['Cart', 'Address', 'Confirm'].map((label, i) => {
            const stepIndex = step === 'cart' ? 0 : step === 'address' ? 1 : 2;
            return (
          ...
          {step === 'cart' && (
            <motion.div key="cart" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={transition}>
              ...
              <div className="space-y-4">
                {items.map(item => (
                  <div key={item.id} className="flex gap-4 rounded-2xl bg-card card-shadow p-3">
                    ...
                    <div className="font-mono-price text-sm font-bold text-foreground self-center">
              ...
              <button
                ...
                className="mt-6 w-full rounded-xl bg-primary py-3.5 text-sm font-medium text-primary-foreground active:scale-[0.98] transition-transform"
              >
                Continue to Address
              </button>
            </motion.div>
              ...
              <form onSubmit={handleAddressSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <input
                    ...
                    className="h-12 rounded-xl bg-card card-shadow px-4 text-foreground placeholder:text-muted-foreground/50 outline-none focus:ring-2 focus:ring-primary transition-all"
                  />
                  <input
                    type="tel"
                ...
                <button
                  type="submit"
                  className="mt-4 w-full rounded-xl bg-primary py-3.5 text-sm font-medium text-primary-foreground active:scale-[0.98] transition-transform"
                >
                  Review Order
                </button>
              </form>
